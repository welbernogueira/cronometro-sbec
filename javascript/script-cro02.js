...
function initializeTimer(timer2, stopwatch3) {
  const timerElement = document.getElementById(`timer${timer2}`);
  const marksList = document.getElementById(`marks-list${timer2}`);
  let intervalId = 0;
  let timer = 0;
  let marks = [];
  let isRunning = false;
  ...
  const toggleTimer = () => {
    const button = document.getElementById(`power${timer2}`);
  ...
  const synchronizeTimers = () => {
    const syncButton = document.getElementById(`power${stopwatch3}`);
